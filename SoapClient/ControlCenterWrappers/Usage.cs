...
namespace SoapClient.ControlCenterWrappers
{
    ...
    {
        internal static void ProcessAccountUsageByCustomer(TerminalService service, string licenseKey, long accountId, DateTime cycleStartDate)
        {
            try
            {
                Trace.WriteLine($"=== Processing Usage for Account {accountId} - Cycle: {cycleStartDate:yyyy-MM-dd} ===");

                List<string> iccids = GetTerminalsForAccount(service, licenseKey, accountId);
                ...
                foreach (string iccid in iccids)
                {
                    try
                    {
                        var usage = GetTerminalUsage(service, licenseKey, iccid, cycleStartDate);
                        ...
                        if (usage != null && terminalDetails != null)
                        {
                            terminalUsages.Add(new TerminalUsageInfo
                            {
                                ...
                                DataUsageMB = usage.billableDataUsage / (1024 * 1024),
                                SmsUsage = usage.billableSmsUsage,
                                VoiceUsageSeconds = usage.billableVoiceUsage
                            });
                        }
                    }
                    catch (Exception ex)
        ...
        private static List<string> GetTerminalsForAccount(TerminalService service, string licenseKey, long accountId)
        {
            ...
            GetTerminalsRequest request = new GetTerminalsRequest()
            {
                ...
                version = "1.0",
                messageId = $"GetTerminals_{accountId}",
                accountId = accountId
            };
            try
            {
                var response = service.GetTerminals(request);
            ...
            throw new NotImplementedException("GetTerminalsForAccount needs to be implemented based on available API methods");
        }

        private static GetTerminalUsageResponse GetTerminalUsage(TerminalService service, string licenseKey, string iccid, DateTime cycleStartDate)
        {
            ...
            try
            {
                // You'll need to add this method to your TerminalService proxy
                // or implement it based on your SOAP service
                var response = service.GetTerminalUsage(request);
                return response;
            }
        ...
        {
            // Implement this to get customer and device info
            ...
            */
            ...
            return new TerminalDetails { Customer = "Unknown", DeviceId = "Unknown" };
        }
